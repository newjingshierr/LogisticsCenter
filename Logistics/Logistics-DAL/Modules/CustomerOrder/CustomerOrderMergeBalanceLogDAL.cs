...
{
    public class CustomerOrderMergeBalanceLogDAL
    {
        public static logistics_customer_order_merge_balance_log GetMergeBalanceLogModel(long ID, long TenantID = BusinessConstants.Admin.TenantID)
        {
            var result = new logistics_customer_order_merge_balance_log();
            MySqlParameter[] parameters = {
                new MySqlParameter("@_TenantID",TenantID),
                new MySqlParameter("@_ID", ID)
            };

            var dbResult = AkmiiMySqlHelper.GetDataSet(ConnectionManager.GetWriteConn(), CommandType.StoredProcedure, Proc.CustomerOrderMerge.logistics_customer_order_merge_balance_log_GetModel, parameters);
            if (dbResult.Tables.Count > 0 && dbResult.Tables[0].Rows.Count > 0)
            {
                result = ConvertHelper<logistics_customer_order_merge_balance_log>.DtToModel(dbResult.Tables[0]);
            }
            else
            {
                result = null;
        ...
        public static bool Insert(logistics_customer_order_merge_balance_log model, AkmiiMySqlTransaction trans = null)
        {
            ...
            MySqlParameter[] parameters = {
                        ...
                        new MySqlParameter("@_CustomerOrderNo",model.CustomerOrderMergeID),
                        new MySqlParameter("@_TransationID",model.TransationID),
                        new MySqlParameter("@_Type",model.Type),
                        new MySqlParameter("@_Direction",model.Direction),
                        new MySqlParameter("@_Amount",model.Amount),
                        new MySqlParameter("@_Orignal",model.Orignal),
                        new MySqlParameter("@_Comment",model.Comment),
                        new MySqlParameter("@_BalanceDate",model.BalanceDate),
            ...
            if (trans == null)
            {
                result = AkmiiMySqlHelper.ExecuteNonQuery(ConnectionManager.GetWriteConn(), CommandType.StoredProcedure, Proc.CustomerOrderMerge.logistics_customer_order_merge_balance_log_ADD, parameters);
            }
            else
            {
                result = AkmiiMySqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, Proc.CustomerOrderMerge.logistics_customer_order_merge_balance_log_ADD, parameters);
            }
            return result == 1;
        ...
        public static bool Delete(long TenantID, long ID, AkmiiMySqlTransaction trans = null)
        {
            ...
            if (trans == null)
            {
                result = AkmiiMySqlHelper.ExecuteNonQuery(ConnectionManager.GetWriteConn(), CommandType.StoredProcedure, Proc.CustomerOrderMerge.logistics_customer_order_merge_balance_log_Delete, parameters);
            }
            else
            {
                result = AkmiiMySqlHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, Proc.CustomerOrderMerge.logistics_customer_order_merge_balance_log_Delete, parameters);
            }
            return result == 1;
